...
10. `[10] import './index.css';`  
   全体共通スタイルを読み込む。背景色、文字色、共通レイアウト、テーマ変数がここから効く。

### ブロック2: 固定値とstate初期化（12〜22行）

```tsx
[12] const sectionIds = ['hero', 'about', 'projects', 'skills', 'contact'];
[14] function App() {
...
1. `[12] const sectionIds = ['hero', 'about', 'projects', 'skills', 'contact'];`  
   監視対象セクションIDの一覧を作る。  
...
3. `[15] const [scrollY, setScrollY] = useState(0);`  
   現在のスクロール位置を保存する状態を作る（初期値0）。  
   目的: スクロールに応じたUI変化に使うため。

4. `[16] const [scrollProgress, setScrollProgress] = useState(0);`  
   ページ読了率（%）を保存する状態を作る（初期値0）。  
   目的: 進捗バーの幅計算に使うため。

5. `[17] const [activeSection, setActiveSection] = useState('hero');`  
   現在アクティブなセクション名を保存する状態を作る（初期値`hero`）。  
   目的: ナビの「今いる位置」の強調表示に使うため。

6. `[18] const [theme, setTheme] = useState<'light' | 'dark'>(() => {`  
   テーマ状態を作る。初期値は固定文字ではなく関数で決める。  
   目的: 保存済みテーマを起動時に読み込むため。

7. `[19] return (localStorage.getItem('theme') as 'light' | 'dark') ?? 'light';`  
   `localStorage`からテーマを取得し、なければ`light`を返す。  
   目的: 前回選んだテーマを復元し、未設定なら安全にライトテーマで開始するため。

8. `[20] });`  
   `theme`初期化関数を閉じる。  
   目的: `[18]-[19]`の初期化定義を完了するため。

9. `[21] const visibleSections = useRef(new Map<string, number>());`  
   セクションごとの可視率を保存する`Map`を`useRef`で作る。  
   目的: 再レンダーのたびに消えない入れ物として保持するため。

補足（分解）:
- `[19]` の `?? 'light'` は「値が`null`なら`light`を使う」という意味。
- `useRef(new Map(...))` は「更新しても即再描画はしないが、値は保持したい」時に使う。

### ブロック3: テーマ反映effect（23〜27行）
...
2. `[24] document.documentElement.setAttribute('data-theme', theme);`  
   HTML全体に`data-theme="light"`または`data-theme="dark"`を付ける。  
   これをCSS側が読んで、色テーマを切り替える。
3. `[25] localStorage.setItem('theme', theme);`  
   今のテーマをブラウザ保存領域に保存する。  
...
### ブロック4: スクロール追跡effect（28〜37行）

```tsx
[28]   useEffect(() => {
[29]     const handleScroll = () => {
[30]       setScrollY(window.scrollY);
[31]       const totalHeight = document.documentElement.scrollHeight - window.innerHeight;
[32]       setScrollProgress(totalHeight > 0 ? (window.scrollY / totalHeight) * 100 : 0);
[33]     };
...
2. `[29] const handleScroll = () => {`  
   スクロールされたときに呼ばれる関数を定義する。
3. `[30] setScrollY(window.scrollY);`  
   現在のスクロール位置（px）を`scrollY`へ保存する。
4. `[31] const totalHeight = document.documentElement.scrollHeight - window.innerHeight;`  
   ページ全体で「実際にスクロール可能な高さ」を計算する。
5. `[32] setScrollProgress(totalHeight > 0 ? (window.scrollY / totalHeight) * 100 : 0);`  
   スクロール率を0〜100%で計算して保存する。  
   `totalHeight > 0` は0で割るのを防ぐための安全策。
6. `[33] };`  
   スクロール時関数の定義を閉じる。
7. `[35] window.addEventListener('scroll', handleScroll);`  
   ブラウザに「スクロール時はhandleScrollを実行して」と登録する。
8. `[36] return () => window.removeEventListener('scroll', handleScroll);`  
   後始末処理。`App`が終了するときに監視を解除する。  
   解除しないと不要な監視が残って不具合の原因になる。
9. `[37] }, []);`  
   処理ブロックを閉じる。`[]`なので初回だけ登録し、以後は再登録しない。

### ブロック5: IntersectionObserver準備（39〜58行）
...
[43]           visibleSections.current.set(entry.target.id, entry.intersectionRatio);
[44]         });
[45]         let maxRatio = 0;
[46]         let maxId = 'hero';
[47]         visibleSections.current.forEach((ratio, id) => {
[48]           if (ratio > maxRatio) {
[49]             maxRatio = ratio;
[50]             maxId = id;
[51]           }
[52]         });
[53]         if (maxRatio > 0) {
[54]           setActiveSection(maxId);
[55]         }
[56]       },
[57]       { threshold: [0, 0.25, 0.5, 0.75, 1] }
[58]     );
```
...
2. `[40] const observer = new IntersectionObserver(`  
   監視オブジェクトを作る。
3. `[41] (entries) => {`  
   見え方が変わったときに呼ばれる処理を定義する（`entries`に監視結果が入る）。
4. `[42] entries.forEach((entry) => {`  
   変化した各要素について順番に処理する。
5. `[43] visibleSections.current.set(entry.target.id, entry.intersectionRatio);`  
   `id`ごとの可視率（0〜1）を`Map`へ保存する。
6. `[45] let maxRatio = 0;`  
   現時点での最大可視率を入れる変数を初期化する。
7. `[46] let maxId = 'hero';`  
   最大可視率だったセクションIDを入れる変数を初期化する。
8. `[47] visibleSections.current.forEach((ratio, id) => {`  
   保存済み可視率をすべて見て、最大値を探す。
9. `[48]-[51] if (ratio > maxRatio) { ... }`  
   今の値が最大値を超えていたら、`maxRatio`と`maxId`を更新する。
10. `[53]-[55] if (maxRatio > 0) { setActiveSection(maxId); }`  
   1つ以上見えている場合だけ、現在セクションを更新する。
11. `[57] { threshold: [0, 0.25, 0.5, 0.75, 1] }`  
   通知タイミングを設定する。  
...
### ブロック6: observer適用と後始末（60〜66行）

```tsx
[60]     sectionIds.forEach((id) => {
[61]       const el = document.getElementById(id);
[62]       if (el) observer.observe(el);
[63]     });
...
分解して読む:
1. `[60] sectionIds.forEach((id) => {`  
   `sectionIds`配列に入っているIDを1つずつ取り出して処理する。
2. `[61] const el = document.getElementById(id);`  
   そのIDを持つ実際のHTML要素を探す。
3. `[62] if (el) observer.observe(el);`  
   要素が見つかったときだけ監視対象として登録する。  
   これで、その要素の見え方が変わると`IntersectionObserver`のコールバックが動く。
4. `[63] });`  
   `forEach`処理を閉じる。
5. `[65] return () => observer.disconnect();`  
...
### ブロック7: テーマ切り替え関数（68〜70行）

```tsx
[68]   const handleThemeToggle = () => {
[69]     setTheme(t => t === 'dark' ? 'light' : 'dark');
[70]   };
```
...
分解して読む:
1. `[68] const handleThemeToggle = () => {`  
   テーマ切り替え専用の関数を作る。  
   目的: クリック時の処理を1つの名前で呼べるようにするため。
2. `[69] setTheme(t => t === 'dark' ? 'light' : 'dark');`  
...
```tsx
[72]   return (
[73]     <div className="app-container">
[74]       <div className="scroll-progress" style={{ width: `${scrollProgress}%` }} />
[75]       <Navbar scrollY={scrollY} theme={theme} onThemeToggle={handleThemeToggle} activeSection={activeSection} />
[76]       <main>
[77]         <Hero />
[78]         <About />
[79]         <Skills />
...
[82]       </main>
[83]       <Footer />
[84]       <ScrollToTop visible={scrollY > window.innerHeight} />
[85]     </div>
[86]   );
[87] }
...
2. `[73] <div className="app-container">`  
   画面全体を包む外側コンテナを開始する。
3. `[74] <div className="scroll-progress" style={{ width: `${scrollProgress}%` }} />`  
   進捗バーを表示する。`scrollProgress`の値に応じてバー幅が変わる。
4. `[75] <Navbar scrollY={scrollY} theme={theme} onThemeToggle={handleThemeToggle} activeSection={activeSection} />`  
   上部ナビを表示し、必要な値を渡す。  
   - `scrollY`: 今のスクロール位置  
   - `theme`: 現在テーマ  
...
8. `[83] <Footer />`  
   フッターを表示する。
9. `[84] <ScrollToTop visible={scrollY > window.innerHeight} />`  
   右下の「上へ戻る」ボタンを表示する。  
...
2. `tagline`や`subtitle`など短い文言を1つ変更
3. 保存する
