...
0. [先に全体像をつかむ](#0-先に全体像をつかむ)
1. [実行順で理解する（ここが本編）](#1-実行順で理解するここが本編)
   - [1-1. index.html](#1-1-indexhtml)
   - [1-2. src/main.tsx（最重要）](#1-2-srcmaintsx最重要)
   - [1-3. src/App.tsx](#1-3-srcapptsx)
   - [1-4. 各コンポーネント](#1-4-各コンポーネントsrccomponentstsx)
   - [1-5. src/data/profile.ts](#1-5-srcdataprofilets)
2. [CSSはどう分かれている？](#2-cssはどう分かれているどこを触れば何が変わるか)
3. [最初の編集体験（5分）](#3-最初の編集体験5分)
4. [つまずきやすいポイント集](#4-つまずきやすいポイント集)
5. [使用ライブラリ（1行版）](#5-使用ライブラリ1行版)
6. [面接Q&A（短縮版）](#6-面接qa短縮版)
7. [最後に（この順で読めばOK）](#7-最後にこの順で読めばok)
